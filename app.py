...
def download_workflows():
    ...
    for name, url in WORKFLOW_URLS.items():
        ...
        try:
            logger.info(f"  ⬇ Downloading {name}.json from {url}")
            ...
            logger.info(f"  ✅ {name}.json downloaded and validated")
            
        except requests.RequestException as e:
            logger.error(f"  ❌ Failed to download {name}.json: {e}")
            raise
        except json.JSONDecodeError as e:
            logger.error(f"  ❌ Invalid JSON in {name}.json: {e}")
            raise
    
    logger.info("="*80)
    logger.info("✅ All workflows downloaded successfully")
    logger.info("="*80)
...
# Download models during image build (with retry logic)
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(f"⚠️ Model download attempt {attempt+1} failed, retrying: {e}")
                time.sleep(5)
            else:
                logger.error(f"❌ Model download failed after {max_retries} attempts")
                raise

image = image.run_function(
    download_models_with_retry,
...
@app.cls(
    ...
    scaledown_window=300,  # ✅ FIXED - Parameter baru sesuai Pydantic 2025-02-24
    timeout=3600,
    keep_warm=1,  # ✅ TAMBAH - Jaga 1 container tetap aktif untuk response cepat
)
class ComfyRunner:
    ...
    def startup(self):
        ...
        try:
            ...
            logger.info(f"ComfyUI process started with PID: {self.proc.pid}")
            
        except Exception as e:
            logger.error(f"❌ Failed to start ComfyUI process: {e}")
            raise RuntimeError(f"ComfyUI startup failed: {e}")
        ...
        for i in range(SERVER_STARTUP_TIMEOUT):
            try:
                ...
                if i % 10 == 0:
                    logger.info(f"⏳ Waiting for ComfyUI server... ({i}/{SERVER_STARTUP_TIMEOUT}s)")
                time.sleep(1)
            except Exception as e:
        ...
        if self.proc.poll() is not None:
            ...
            logger.error("="*80)
            logger.error("❌ ComfyUI process exited unexpectedly")
            logger.error(f"STDOUT: {stdout}")
            logger.error(f"STDERR: {stderr}")
            logger.error("="*80)
            raise RuntimeError("ComfyUI process crashed during startup")
...
# app.py: Ganti dengan versi yang lebih simpel
    ...
    def shutdown(self):
        ...
        logger.info("="*80)
        logger.info("🛑 Shutting down ComfyUI container...")
        logger.info("="*80)
        ...
        try:
            if hasattr(self, 'proc') and self.proc:
                ...
                try:
                    ...
                    logger.info("✅ ComfyUI process terminated gracefully")
                except subprocess.TimeoutExpired:
                    logger.warning("⚠️ Process didn't terminate, forcing kill...")
                    self.proc.kill()
                    self.proc.wait()
...
async def get_metrics():
    ...
    try:
        ...
        return {
            "timestamp": time.time(),
            "comfyui_status": health_data.get("comfyui_status"),
            "available_workflows": list(WORKFLOW_MAPPINGS.keys()),
            "total_workflows": len(WORKFLOW_MAPPINGS)
        }
    except Exception as e:
